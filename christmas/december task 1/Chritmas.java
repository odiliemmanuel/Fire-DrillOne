import java.util.Scanner;
    public class Chritmas {
        public static void main(String...Odili) {
        
        firstDay();
        secondDay();
        thirdDay();
        fourthDay();
        fifthDay();
        sixthDay();
        seventhDay();
        eigthDay();
        ninthDay();
        tenthDay();
        eleventhDay();
        twelfthDay();

    }


    public static void firstDay(){
        System.out.println("On the first day of christmas my true love sent to me");
        firstDayBody();
    }

    public static void firstDayBody(){
        System.out.println("A patridge in a pear tree\n");
    }

     public static void secondDay(){
        System.out.println("On the second day of christmas my true love sent to me");
        secondDayBody();
    }

    public static void secondDayBody(){
        System.out.println("Two turtle doves");
        firstDayBody();
    }

     public static void thirdDay(){
        System.out.println("On the third day of christmas my true love sent to me");
        thirdDayBody();
    }

    public static void thirdDayBody(){
        System.out.println("Three french hens");
        secondDayBody();
    }


    public static void fourthDay(){
        System.out.println("On the fourth day of christmas my true love sent to me");
        fourthDayBody();
    }

    public static void fourthDayBody(){
        System.out.println("Four colly birds");
        thirdDayBody();
    }

    
      public static void fifthDay(){
        System.out.println("On the fifth day of christmas my true love sent to me");
        fifthDayBody();
    }

    public static void fifthDayBody(){
        System.out.println("Five golden rings");
        fourthDayBody();
    }


      public static void sixthDay(){
        System.out.println("On the sixth day of christmas my true love sent to me");
        sixthDayBody();
    }

    public static void sixthDayBody(){
        System.out.println("Six geese a-laying");
        fifthDayBody();
    }

    

      public static void seventhDay(){
        System.out.println("On the seventh day of christmas my true love sent to me");
        seventhDayBody();
    }

    public static void seventhDayBody(){
        System.out.println("Seven swans a-swimming");
        sixthDayBody();
    }


      public static void eigthDay(){
        System.out.println("On the eight day of christmas my true love sent to me");
        eightDayBody();
    }

    public static void eightDayBody(){
        System.out.println("Eight maids a-milking");
        seventhDayBody();
    }


    
      public static void ninthDay(){
        System.out.println("On the eight day of christmas my true love sent to me");
        ninthDayBody();
    }

    public static void ninthDayBody(){
        System.out.println("Nine ladies dancing");
        eightDayBody();
    }

    
    
     public static void tenthDay(){
        System.out.println("On the eight day of christmas my true love sent to me");
        tenthDayBody();
    }

    public static void tenthDayBody(){
        System.out.println("Ten lords a-leaping");
        ninthDayBody();
    }

     
     public static void eleventhDay(){
        System.out.println("On the eight day of christmas my true love sent to me");
        eleventhDayBody();
    }

    public static void eleventhDayBody(){
        System.out.println("Eleven pipers piping");
        tenthDayBody();
    }



    public static void twelfthDay(){
        System.out.println("On the eight day of christmas my true love sent to me");
        twelfthDayBody();
        System.out.println("SEE YOU NEXT TIME 🥳️");
    }

    public static void twelfthDayBody(){
        System.out.println("Twelve drummers drumming");
        eleventhDayBody();
    }

    







    }

     










